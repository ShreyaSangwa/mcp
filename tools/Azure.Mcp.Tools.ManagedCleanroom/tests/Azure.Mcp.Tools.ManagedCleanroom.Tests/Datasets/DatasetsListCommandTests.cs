...
public sealed class DatasetsListCommandTests : CommandUnitTestsBase<DatasetsListCommand, IManagedCleanroomServiceDataPlane>
{
    ...
    {
        var expected = JsonDocument.Parse("""{"datasets":[{"documentId":"a1b2c3d4","datasetName":"my-dataset","status":"Published"}]}""").RootElement;
        Service.ListDatasetsAsync(
            Arg.Any<string>(), Arg.Any<string>(), Arg.Any<bool>(), Arg.Any<string?>(), Arg.Any<CancellationToken>())
            .Returns(expected);
        ...
        result.AssertProperty("datasets");
    }
    ...
    public async Task ExecuteAsync_ReturnsServiceResponse()
    {
        ...
        await Service.Received(1).ListDatasetsAsync(
