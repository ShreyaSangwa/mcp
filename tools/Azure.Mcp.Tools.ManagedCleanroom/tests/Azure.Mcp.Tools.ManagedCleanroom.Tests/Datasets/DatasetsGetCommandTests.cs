...
public sealed class DatasetsGetCommandTests : CommandUnitTestsBase<DatasetsGetCommand, IManagedCleanroomServiceDataPlane>
{
    ...
    {
        var expected = JsonDocument.Parse("""{"documentId":"a1b2c3d4-e5f6-7890-abcd-ef1234567890","status":"Published","datasetName":"my-dataset"}""").RootElement;
        Service.GetDatasetAsync(
            Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<bool>(), Arg.Any<string?>(), Arg.Any<CancellationToken>())
            .Returns(expected);
    ...
    public async Task ExecuteAsync_ReturnsServiceResponse()
    {
        ...
        await Service.Received(1).GetDatasetAsync(
