...
public sealed class AnalyticsGetCommandTests : CommandUnitTestsBase<AnalyticsGetCommand, IManagedCleanroomServiceDataPlane>
{
    ...
    {
        var expected = JsonDocument.Parse("""{"analyticsEndpoint":"https://analytics.example.com","status":"Active"}""").RootElement;
        Service.GetAnalyticsAsync(
            Arg.Any<string>(), Arg.Any<string>(), Arg.Any<bool>(), Arg.Any<string?>(), Arg.Any<CancellationToken>())
            .Returns(expected);
        ...
        result.AssertProperty("analyticsEndpoint");
    }
