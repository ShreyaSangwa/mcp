...
public sealed class CollaborationsGetCommandTests : CommandUnitTestsBase<CollaborationsGetCommand, IManagedCleanroomServiceDataPlane>
{
    ...
    public async Task ExecuteAsync_ValidatesInputCorrectly(string args, bool shouldSucceed)
    {
        if (shouldSucceed)
        {
            Service.GetCollaborationAsync(
                Arg.Any<string>(), Arg.Any<string>(), Arg.Any<bool?>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<Microsoft.Mcp.Core.Options.RetryPolicyOptions?>(), Arg.Any<CancellationToken>())
    ...
    {
        var expected = JsonDocument.Parse("""{"collaborationId":"c1","collaborationName":"test","userStatus":"Active"}""").RootElement;
        Service.GetCollaborationAsync(
            Arg.Any<string>(), Arg.Any<string>(), Arg.Any<bool?>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<Microsoft.Mcp.Core.Options.RetryPolicyOptions?>(), Arg.Any<CancellationToken>())
            .Returns(expected);
        ...
        result.AssertProperty("collaborationId");
    }
    ...
    public async Task ExecuteAsync_ReturnsServiceResponse()
    {
        ...
        await Service.Received(1).GetCollaborationAsync(
            TestEndpoint, TestCollaborationId, null, null, null, null, Arg.Any<CancellationToken>());
    }

    [Fact]
    public async Task ExecuteAsync_WithIncludeDeleted_PassesFlagThrough()
    {
        Service.GetCollaborationAsync(
            Arg.Any<string>(), Arg.Any<string>(), Arg.Any<bool?>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<Microsoft.Mcp.Core.Options.RetryPolicyOptions?>(), Arg.Any<CancellationToken>())
            .Returns(default(JsonElement));
        ...
        Assert.Equal(HttpStatusCode.OK, response.Status);
        await Service.Received(1).GetCollaborationAsync(
            TestEndpoint, TestCollaborationId, true, null, null, null, Arg.Any<CancellationToken>());
    }
